...
| `nnUNetTrainerDA5Septal{TverskyV2,OversampleTverskyV2}` + `_200e/_250e` | `nnUNetTrainerDA5SeptalAblation.py` | **FIXED Tversky** arms. Dataset070 ablation showed the original weight-1.0 Tversky *collapsed* class 8 to 0 predicted voxels (0/13 test cases vs oversample-only 10/13). V2 = weight 0.1 + warmup (term off until ep50, or ep100 in the `_250e` set) + linear ramp (30 ep) + softer bias (α/β 0.4/0.6). Original arms kept for reproducibility. **Oversample-only was the winning arm** (10/13 cases, val pseudo-Dice 0.22 vs baseline 0.16). The `_250e` matched-budget set (also `nnUNetTrainerDA5_250epochs`, `...SeptalOversample_250epochs`) exists because PolyLR is tied to `num_epochs` — a fair long-schedule comparison must rerun every arm at the same budget. |
...
| Dataset071 | 71 | `Dataset071_ImageCHDClinicalOrientation` | **Clinical-orientation (LPS)** anatomy set: images from Dataset060, **clean 7-class labels from Dataset030 (NO septal id 8)**, physically reoriented RAS→LPS | Yes | The "train and send to clinic" anatomy model — realizes the orientation step of the deferred *Clinician dataset*. **Pools both source Tr+Ts** (ImageCHD's split is arbitrary) and re-partitions the output **by myocardium presence** (myo→train, no-myo→holdout) so every training case has real myo. True `sitk.DICOMOrient` reorient (voxel flip, not header edit); no HU shift, no resampling (nnU-Net does spacing later). Built by `tools/build_dataset071_clinical_orientation.py` (verifies LPS + geometry + lossless labels/HU; `--no-myo-partition` to follow source folders instead). Train with `scripts/CHD_Dataset071_clinical_orientation.sh` (preprocess → reuse Dataset060 5-fold split with a case-set match guard → DA5 `_200epochs`+`_500epochs`, all 5 folds; resumable per fold). |
| Dataset072 | 72 | `Dataset072_ImageCHDMultiFOV` | **Multi-FOV** derived from Dataset071 (LPS): per case a full copy **+** cardiac-bbox crops at **60 mm** and **30 mm**/side (`<case>_full`/`_bbox60mm`/`_bbox30mm`) | Yes | Domain-adaptation for clinical inference (clinical CT has tight cardiac FOV; full ImageCHD does not). **Crop-only** sub-region extraction — spacing/HU/labels preserved exactly, NO resample/upsample (no invented detail); FOV/scale diversity shifts nnU-Net's shape fingerprint toward clinical. Deterministic v1 (no random aug). Built by `tools/build_dataset072_multifov.py` (QA: `qa_percase.csv`, `qa_summary.json`; `case_groups.json` for **grouped CV** so a patient's variants don't straddle train/val). One-shot run: `scripts/CHD_Dataset072_multifov.sh` (build → preprocess → **overwrite `splits_final.json` with a grouped 5-fold** (no per-patient leakage) → DA5 train). |
