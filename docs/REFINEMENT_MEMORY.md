...
## Latest correction — reuse D090, preserve chamber LCCs

User correctly questioned redundant baseline training. D090 is the matched original-50 baseline, while D091 adds nine cases. The launcher now reuses D090 and constructs/trains/evaluates only D095–D097. D094 from the preliminary design is not created. Preflight checks current D090 model/source metadata, required checkpoints and existing D080 prediction case completeness, and records hashes. It cannot retrospectively prove old training bytes or old prediction provenance. Build additionally requires selected seeds to be byte-identical to D090 source labels and copied plans unchanged.

The earlier 70% retention guard could still remove 30% of a good chamber. Default nnInteractive composition now unions each chamber candidate with its entire original LCC, restricts additions to original background, removes disconnected additions and abstains on competing background claims. Original chamber voxels cannot be lost; an assertion enforces this. Growth cap remains 4× as a provisional guardrail, not anatomical validation. This cannot remove existing false positives and may still add erroneous foreground. Optional replacement mode is explicit. Raw nnInteractive counts and final additions remain separately auditable.

Policy changed: default run directory is `accepted50_conservative_v3`. Already submitted v2 jobs are not automatically cancelled or modified; do not mix their results with v3. Regression tests cover empty chamber candidates preserving their seeds through final assembly.
