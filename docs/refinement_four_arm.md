...
|---|---|---|
| `baseline` | 94 | None; original LCC masks |
...
Myocardium (5) remains unchanged. All four training cohorts contain the same 97 ImageCHD + 50 accepted pseudo cases, **147 total**. The existing five ImageCHD validation splits, architecture, spacing, patch size and intensity normalization are copied from D090. No clinical-spacing experiment or replanning is included. Pseudo cases stay train-only. Existing dataset IDs are refused; override all four together if these IDs are occupied.
...
The launcher queues preparation → preprocessing → training → D080 prediction/scoring with `afterok` dependencies. The default is **all five folds and the five-fold ensemble**, matching the earlier D091/D092 workflow. Run on the login node:
...
```text
nnUNet_raw/Dataset080_ClinicalCaseSanjibDetailed/predictions/ds094_fold0/
.../ds094_fold1/ through ds094_fold4/ and ds094_ensemble/
...
For the trained-model comparison, use the same held-out image inputs, inference route, postprocessing and fold selection for all four datasets. Predictions should use trainer `nnUNetTrainerDA5_200epochs`, plans `nnUNetResEncUNetMPlans`, configuration `3d_fullres`; compare fold 0 against fold 0, then corresponding folds/ensembles. Do not compare a single refined fold against a baseline five-fold ensemble. The existing D090 checkpoint is historical context, not a guaranteed substitute for a newly matched baseline. Optimization randomness is not fixed by this runner.
...
For nnInteractive chambers, the candidate replaces the original chamber only if cleanup leaves at least 70% of its seed voxels and volume at most 4× the seed; otherwise that chamber falls back. No seed / empty usable proposal also triggers fallback. Technical failures (wrong geometry, missing files, failed model command) stop the stage rather than count as a successful fallback.
