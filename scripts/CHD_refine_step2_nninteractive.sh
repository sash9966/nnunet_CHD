...
for lab in "$LCC_DIR"/*.nii.gz; do
  ...
  echo "==== $c ===="
  # ALL structures: chambers via lasso, Myo via spread points (+ chamber negatives), Aorta/PA via centerline points
  python tools/run_nninteractive_refine.py --image "$img" --label "$lab" --out "$out" \
      --structures LV-BP,RV-BP,LA,RA,Myo,Aorta,Pulmonary --prompt-mode lasso \
