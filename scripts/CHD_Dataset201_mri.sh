...
#SBATCH --output=/scratch/users/sastocke/nnunet_CHD/logs/D201-MRI-all_%j.out
#SBATCH --error=/scratch/users/sastocke/nnunet_CHD/logs/D201-MRI-all_%j.err
...
mkdir -p "${RUN_RECORD}"
cp "scripts/CHD_Dataset201_mri.sh" "${RUN_RECORD}/"
stamp_provenance "D201-all" "${RUN_RECORD}" \
...
for FOLD in "${FOLDS[@]}"; do
  ...
  stamp_provenance "D201-all-fold${FOLD}" "${OUT}" "TRAINER=${TRAINER}" "PLANS=${PLANS}" "RUN_RECORD=${RUN_RECORD}" "nnUNet_compile=${nnUNet_compile}"
...
stamp_provenance "D201-MRI-export-all" "${EXPORT}" "TRAINER=${TRAINER}" "PLANS=${PLANS}" "RUN_RECORD=${RUN_RECORD}" "nnUNet_compile=${nnUNet_compile}"
cat > "${EXPORT}/HOW_TO_USE.txt" <<USAGE
Dataset201_ClinicalBostonMRI: 27 MRI cases, seven foreground classes, DA5 100 epochs, fold all.
All cases were used for training. Any fold_all validation is IN-SAMPLE.
Five-fold validation contains clinical patients only; clinical review is required.
Native MRI prediction (CASE_0000.nii.gz; no CT-grid resize):
  nnUNetv2_predict -i <mri_dir> -o <out_dir> -d 201 -c ${FULLRES} -tr ${TRAINER} -p ${PLANS} -f all -chk checkpoint_final.pth
...
echo 'Next: sbatch scripts/CHD_Dataset201_train5fold.sh'
