#!/bin/bash
# =============================================================================
#  CHD_Dataset202_mri.sh
#  Dataset202_ClinicalBostonPseudoMRI: Clinical + reviewed Boston + reviewed pseudo myocardium; ResEnc-M / DA5 / 100 epochs.
...
#SBATCH --output=/scratch/users/sastocke/nnunet_CHD/logs/D202-MRI-all_%j.out
#SBATCH --error=/scratch/users/sastocke/nnunet_CHD/logs/D202-MRI-all_%j.err
...
DATASET_ID=202
DATASET_NAME="Dataset202_ClinicalBostonPseudoMRI"
PLANNER="nnUNetPlannerResEncM"; PLANS="nnUNetResEncUNetMPlans"; FULLRES="3d_fullres"
TRAINER="nnUNetTrainerDA5CaseWeighted_100epochs"; FOLDS=(all)
...
mkdir -p "${RUN_RECORD}"
cp "scripts/CHD_Dataset202_mri.sh" "${RUN_RECORD}/"
stamp_provenance "D202-all" "${RUN_RECORD}" \
...
[ -f "${nnUNet_raw}/${DATASET_NAME}/dataset.json" ] || { echo 'Dataset202_ClinicalBostonPseudoMRI is missing from nnUNet_raw' >&2; exit 1; }
...
p = Path(os.environ['nnUNet_raw']) / 'Dataset202_ClinicalBostonPseudoMRI'
...
assert images == labels and len(images) == m['numTraining'], 'Image/label count mismatch'
weights = json.loads((p / 'case_weights.json').read_text())
assert set(weights) == images and all(isinstance(v, (float, int)) and 0 < v < float('inf') for v in weights.values())
CHECK
...
# Freeze weights with preprocessing; prevents a stale preprocessed map overriding raw weights.
cp "${nnUNet_raw}/${DATASET_NAME}/case_weights.json" "${nnUNet_preprocessed}/${DATASET_NAME}/case_weights.json"
cp "${nnUNet_raw}/${DATASET_NAME}/case_weights.json" "${RUN_RECORD}/"
flock -u 9
exec 9>&-
...
for FOLD in "${FOLDS[@]}"; do
  ...
  stamp_provenance "D202-all-fold${FOLD}" "${OUT}" "TRAINER=${TRAINER}" "PLANS=${PLANS}" "RUN_RECORD=${RUN_RECORD}" "nnUNet_compile=${nnUNet_compile}"
...
stamp_provenance "D202-MRI-export-all" "${EXPORT}" "TRAINER=${TRAINER}" "PLANS=${PLANS}" "RUN_RECORD=${RUN_RECORD}" "nnUNet_compile=${nnUNet_compile}"
cat > "${EXPORT}/HOW_TO_USE.txt" <<USAGE
Dataset202_ClinicalBostonPseudoMRI: reviewed expanded MRI cohort, seven foreground classes, DA5 100 epochs, fold all.
All cases were used for training. Any fold_all validation is IN-SAMPLE.
No unbiased clinical CV: fold_all teacher saw all clinical training patients. Evaluate new clinical patients.
Native MRI prediction (CASE_0000.nii.gz; no CT-grid resize):
  nnUNetv2_predict -i <mri_dir> -o <out_dir> -d 202 -c ${FULLRES} -tr ${TRAINER} -p ${PLANS} -f all -chk checkpoint_final.pth
...
echo 'Evaluate this model on the new clinical cohort; no ordinary five-fold run for teacher-derived labels.'
