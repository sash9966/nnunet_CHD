#!/bin/bash
# =============================================================================
#  CHD_Dataset051_disease_landmarks.sh  (v2 septal-defect derivation)
#  Dataset051_imageCHD_DiseaseLandmarksV2 — 7 anatomy labels + unified
#  septal-defect label (id 8), v2 derivation: unified VSD+ASD+AVSD-cross
#  (LV-RV minus myo, LA-RA + LV-RA/RV-LA direct adjacency). A/B vs Dataset050 (v1).
#
#  Regular nnU-Net pipeline, 3d_fullres, fold 0:
#     Phase 0   build Dataset051 from Dataset030 (chd_landmarks; source read-only)
...
# =============================================================================
#SBATCH --job-name=D051-landmarks
...
#SBATCH --output=/scratch/users/sastocke/nnunet_CHD/logs/D051-landmarks_%j.out
#SBATCH --error=/scratch/users/sastocke/nnunet_CHD/logs/D051-landmarks_%j.err
...
CKPT_DIR="${nnUNet_results}/${DATASET_NAME}/.checkpoints/CHD_Dataset051_disease_landmarks"
mkdir -p "${CKPT_DIR}" "${PRED_BASE}" /scratch/users/sastocke/nnunet_CHD/logs
cd "${REPO}"

# ─────────────────────────────────────────────
# Phase 0 — build Dataset051 (source Dataset030 READ-ONLY)
# ─────────────────────────────────────────────
if [ ! -f "${CKPT_DIR}/00_build.done" ]; then
  ...
  python -m chd_landmarks.cli build-dataset \
      ...
      --target-dataset-name imageCHD_DiseaseLandmarksV2 \
      --metadata "${METADATA}" \
...
# ─────────────────────────────────────────────
# Phase 1 — plan & preprocess (3d_fullres only; Dataset051 ONLY)
# ─────────────────────────────────────────────
...
#   Dataset051 has the same case IDs as Dataset030, so copying splits_final.json
