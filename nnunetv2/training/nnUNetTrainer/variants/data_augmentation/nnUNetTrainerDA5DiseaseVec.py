"""
DA5 trainer variant with disease-vector conditioning.

Loads ``disease_map.json`` from the preprocessed dataset folder and injects
a per-sample disease vector into the network at every forward pass.
When the JSON is absent, training falls back to exact baseline behaviour.
...
class nnUNetTrainerDA5DiseaseVec(nnUNetTrainerDA5):
    """nnUNetTrainerDA5 + disease-vector conditioning.
    ...
    def build_network_architecture(  # type: ignore[override]
        ...
        # 2) resolve conv / norm / nonlin ops from the plans kwargs
        ...
        wrapped.disease_mlp.apply(DiseaseConditionedResEncUNet.initialize)
        wrapped.bottleneck_injector.apply(DiseaseConditionedResEncUNet.initialize)
        wrapped.decoder_injectors.apply(DiseaseConditionedResEncUNet.initialize)
    ...
    # ------------------------------------------------------------------
    # Checkpoint loading  (handle missing disease keys gracefully)
    # ------------------------------------------------------------------
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        # use strict=False so vanilla checkpoints can load into the disease-conditioned network
        # (disease_mlp + injector weights will be randomly initialised)
        mod = self._get_unwrapped_network()
...
class nnUNetTrainerDA5DiseaseVec_100epochs(nnUNetTrainerDA5DiseaseVec):
    def __init__(
        self,
        plans: dict,
        configuration: str,
